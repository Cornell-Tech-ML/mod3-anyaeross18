...
class IsClose(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor, t2: Tensor) -> Tensor:
        """Returns true if the tensors are close"""
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        newOrder = [
            a[0]
            for a in sorted(
                enumerate(order[x] for x in range(order.size)), key=lambda x: x[1]
            )
        ]
        return grad_output._new(grad_output._tensor.permute(*newOrder)), 0.0
...
def grad_central_difference(
    ...
    """Check the gradient using central difference."""
    x = vals[arg]
    up = zeros(x.shape)
    up[ind] = epsilon
